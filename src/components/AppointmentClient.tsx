  ...
  const handleNext = async () => {
    if (step === 3) {
      ...
      try {
        ...
        if (res.ok) setStep(4);
        else alert("Failed to book appointment. Please try again.");
      } catch (err) {
         console.error(err);
         alert("Network error.");
      } finally {
              ...
              <div className="p-8 md:p-14">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-4">
                        ...
                        >
                          ...
                          {initialDepartments.map((d: any) => <option key={d.id} value={d.id}>{d.title}</option>)}
                        </select>
                      </div>
                      ...
                      <div className="space-y-4">
                        ...
                        >
                          ...
                          {initialDoctors.map((d: any) => <option key={d.id} value={d.id}>{d.name}</option>)}
                        </select>
                      </div>
                    </div>
                ...
                {step === 4 && (
                   ...
                   >
                     ...
                     <div className="bg-gray-50 rounded-3xl p-8 max-w-md mx-auto mb-12 border border-gray-100">
                        <div className="grid grid-cols-2 gap-6 text-left">
                           ...
                           <div>
                             ...
                             <p className="font-bold text-sm text-medical-dark">{initialDoctors.find((d: any) => d.id === formData.doctor)?.name}</p>
                           </div>
                           <div>
                     ...
                     <Link 
                       href="/"
                       className="px-10 py-5 bg-white border border-gray-200 text-medical-dark rounded-2xl font-bold hover:bg-gray-50 transition-all shadow-xl shadow-gray-200/20 inline-block"
                     >
                       Return to Home
                     </Link>
                   </motion.div>
